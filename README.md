...
## النشر (الأسهل والموصى به): Render — بضغطة واحدة

كل البنية معرّفة في `render.yaml`: خادم Next.js **وقاعدة PostgreSQL** يُنشآن ويُربطان تلقائياً، مع تشغيل الهجرات وseed الإدارة. لا تتعامل مع أي تفاصيل قاعدة بيانات.

1. أنشئ حساباً في **render.com** (يمكن الدخول بحساب GitHub).
2. **New ← Blueprint** ← اختر مستودع `mhwar/khuttar` ← Render يقرأ `render.yaml`.
3. سيطلب منك إدخال **شيئين فقط**: `SEED_ADMIN_EMAIL` و`SEED_ADMIN_PASSWORD` (بريد وكلمة مرور حساب الإدارة، 12 حرفاً فأكثر) ← **Apply**.
4. ينشئ Render القاعدة، يحقن رابطها، يبني، يهاجر، يزرع حساب الإدارة، ويُشغّل الموقع على `https://khuttar.onrender.com`. كل push إلى `main` بعدها ينشر تلقائياً.

بعد النشر: ادخل `/login` بحساب الإدارة ← غيّر كلمة المرور من `/admin/users` ← أضف الوكلاء من `/admin/agents` ← ابنِ الوجهات والبرامج من اللوحة.

> **التكلفة**: الخطة المجانية للتجربة (الخادم ينام بعد خمول، وقاعدة Render المجانية تُحذف بعد ٣٠ يوماً). للإنتاج رقِّ من `render.yaml` إلى `starter` للخادم و`basic-256mb` للقاعدة (~١٤$ شهرياً). الدومين الرسمي: من إعدادات الخدمة في Render ← Custom Domain.
>
> **بديل بقاعدة Supabase**: إن رغبت في لوحة Supabase، أنشئ مشروع Supabase وضع رابطه المباشر (Session pooler) في متغيّر `DATABASE_URL` على خدمة Render بدل قاعدة Render — والباقي كما هو.

---

## بديل: النشر على Cloudflare Workers

> ملاحظة: هذا مسار بديل أكثر تعقيداً (Workers + Hyperdrive، ويتطلب Workers Paid). إن اخترت Render أعلاه فتجاهل هذا القسم تماماً. الـ workflow الخاص به أصبح **يدوياً فقط** ولن يعمل تلقائياً.
