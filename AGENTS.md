...
Arabic-first (RTL) platform: Next.js 16 App Router + Tailwind v4 + shadcn/ui + Prisma 6 + PostgreSQL.
...
- **Authz is in layouts AND every mutating action** (`requireAdmin` / `requireAgent` / `requireApprovedAgent` from `lib/auth.ts`); `proxy.ts` is a cookie-presence UX redirect only. Agent queries always filter by their own `agentId`/`ownerAgentId`.
